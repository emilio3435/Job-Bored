#!/usr/bin/env node
// Purpose: Cleanly remove the worker-autostart service installed by
// scripts/install-discovery-worker-autostart.mjs. Idempotent.
//
// Mirrors scripts/uninstall-keep-alive.mjs.
...
} from "./install-discovery-worker-autostart.mjs";
...
function uninstallDarwin(options) {
  ...
  run("launchctl", ["remove", WORKER_AUTOSTART_LABEL], options);
  if (existed) {
...
function uninstallLinux(options) {
  ...
  run(
    "systemctl",
    ["--user", "disable", "--now", `${WORKER_AUTOSTART_LABEL}.service`],
    options,
  );
...
export function uninstallDiscoveryWorkerAutostart(options = {}) {
