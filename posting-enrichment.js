...
function _toastForLlmError(err) {
  ...
  if (/\b(429|RESOURCE_EXHAUSTED|quota|rate)\b/i.test(msg)) {
    host().showToast("AI provider quota or rate limit reached — try again in a minute.", "info");
    return;
  }
...
/** The single entry point. Always succeeds when the selected AI provider is
...
/* When the v2 Dossier opens a role, mirror the legacy drawer's auto-fetch:
   ...
   enrichment yet, kick off the same scrape + AI enrichment that the
...
/* Auto-enrich on Dossier open. The self-healing pipeline in
   ...
   as long as an AI provider is configured, so we no longer gate
