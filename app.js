...
window.JobBoredDiscoveryWizard.ui.host = {
  ...
  handleDiscoveryWizardAction,
  moveDiscoveryWizardToStep,
  probeNgrokFromLocalApi,
};

// Thin delegating wrappers: these wizard helpers now live in
// discovery-wizard-ui.js, but app.js code (settings + the Phase-1b
// render/handle functions that have not moved yet) still calls them by bare
// name. The wrappers preserve those call sites unchanged.
function getDiscoveryWizardActionableStep(...args) {
  return window.JobBoredDiscoveryWizard.ui.getDiscoveryWizardActionableStep(
    ...args,
  );
}
...
function getDiscoveryWizardStepTitle(...args) {
  return window.JobBoredDiscoveryWizard.ui.getDiscoveryWizardStepTitle(...args);
}
function buildDiscoveryWizardSteps(...args) {
  return window.JobBoredDiscoveryWizard.ui.buildDiscoveryWizardSteps(...args);
}
...
async function toggleFavorite(stableKey) {
  ...
  showToast("Saved locally — retry when reconnected", "error");
  return true;
}
...
async function markStatusExpired(stableKey) {
  ...
  try {
    ...
    showToast("Marked Expired", "info");
  } catch (err) {
    console.error("[JobBored] markStatusExpired failed", err);
    job.status = prevStatus;
    renderPipeline();
    showToast("Couldn't mark expired — reverted", "error");
  }
}
...
// or the list re-renders, since indexes are re-derived from pipelineData.
    ...
    .map((entry) => {
      ...
      return `
        <article class="expired-review-item" data-row-index="${entry.index}">
          <input type="checkbox" class="expired-review-item__select" data-action="expired-review-select" data-index="${entry.index}" ${checked} aria-label="Select ${escapeHtml(title)}" />
          ...
          <div class="expired-review-item__actions">
            ${
              href
                ? `<a class="expired-review-item__action" href="${escapeHtml(href)}" target="_blank" rel="noopener">Open posting</a>`
                : ""
            }
...
async function applyExpiredReviewAction(action, index) {
  ...
  if (action === "dismiss") {
    await dismissJob(index);
    return;
  }
  const sheetRow = getSheetRow(index);
  if (!sheetRow) return;
  let nextStatus = null;
  ...
  try {
    ...
    job.status = prevStatus;
    renderPipeline();
    showToast(`Couldn't update status — reverted`, "error");
  }
}
