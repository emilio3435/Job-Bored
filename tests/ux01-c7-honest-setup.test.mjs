...
describe("C7 · the recommended provider pins a capable model (FR-07)", () => {
  it("B2's OpenRouter default is not a :free model the app calls weak", () => {
    const env = loadArrival();
    const model = env.beats.ai.defaultModelFor("openrouter");
    assert.equal(model, "openai/gpt-5.4-mini");
    assert.equal(env.window.JobBoredModelCatalog.isWeakMaterialsModel(model), false);
    assert.equal(
      env.beats.ai.PROVIDERS.find((p) => p.id === "openrouter").defaultModel,
      undefined,
