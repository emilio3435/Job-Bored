...
**Empty-result fallback:** if the allowlist matches zero catalog companies, the request is `blocked_unresolved` and fails preflight. The worker does not run unrestricted grounded-web, ATS-memory, or ATS-host fallback lanes. Broad fallback is allowed only when the request explicitly sets `allowUnrestrictedFallback: true`. When that flag is true and the stored **active** company list plus history are empty, the unmatched allowlist names become ephemeral company targets for this run only (they are not persisted). This is how a post-wizard dashboard allowlist produces a non-zero Companies Seen count instead of a blank-company unrestricted scout.
