...
**Status:** authoritative contract for the `feat/companies-run-backend` and `feat/companies-run-frontend` workspaces. Both implementations must conform to this document; do not diverge without updating the doc first and notifying the orchestrator.

**Scope:** the ephemeral per-run company override described in the approved plan at `/Users/emilionunezgarcia/.claude/plans/lazy-giggling-goblet.md`. Plan is authoritative for *what* to build; this doc is authoritative for the wire format and server semantics *between* the two workspaces.
