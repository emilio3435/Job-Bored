...
const reqs = new Map();
cdp.on("Network.responseReceived", (e) => { reqs.set(e.requestId, { url: e.response.url, mime: e.response.mimeType, type: e.type, status: e.response.status, ce: e.response.headers["content-encoding"] || e.response.headers["Content-Encoding"] || "" }); });
...
if (!process.env.NOCOV) await page.coverage.startJSCoverage({ resetOnNavigation: false });
if (!process.env.NOCOV) await page.coverage.startCSSCoverage({ resetOnNavigation: false });
await page.reload({ waitUntil: "load" });
await page.waitForLoadState("networkidle").catch(() => {});
await page.waitForTimeout(2500);
const js = process.env.NOCOV ? [] : await page.coverage.stopJSCoverage();
const css = process.env.NOCOV ? [] : await page.coverage.stopCSSCoverage();
...
const dedupe = (entries, isCss) => { const m = new Map(); for (const e of entries) { const len = (isCss ? e.text : e.source)?.length || 0; let u = 0; if (isCss) for (const r of e.ranges) u += r.end - r.start; const k = e.url; const prev = m.get(k); if (!prev || (isCss ? u > prev._u : true)) { e._u = u; m.set(k, e); } } return [...m.values()]; };
const jc = cov(dedupe(js, false), false), cc = cov(dedupe(css, true), true);
