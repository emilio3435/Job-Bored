...
## Routed from the orchestrator's review of the V1/V2 screenshots (2026-09-02)
- **U3 · Beat 4 mobile · UGLY** — in `after-beat4-390x844.png` the "Add" buttons beside the role/strength inputs wrap to "Ad / d". Assert (and fix inside `css/oneflow.css` CORE, granted) that inline action buttons never wrap below 480 px; pin with a bounding-box check.
- **C1 re-tag:** boot-error's RCA found the cold-start `appendChild` pageerror does not reproduce; treat C1 as a walkthrough artifact and keep only that lane's null-guard pin. Verify the app itself, not Gemini's screenshots (the report has prose/media disagreements).
