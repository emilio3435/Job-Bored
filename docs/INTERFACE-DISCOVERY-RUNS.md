...
| J | `Error` | `timeout on acme.com` | Blank when `status=success`. On `partial` / `failure`, the first warning, write error, or lifecycle reason (short, < 200 chars). Never leave Partial-with-zeros blank. |
...
| `Companies Seen` | `config.companies.length` (user-configured company list; 0 after greenfield onboarding until a profile refresh persists companies) | `companies.length` returned by `discoverCompaniesForProfile()`. |
