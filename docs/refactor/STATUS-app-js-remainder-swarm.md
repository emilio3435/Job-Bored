...
> Orchestrator ledger. **Last updated:** 2026-06-01 (session 38 — legacy discovery CSS split landed).
...
| `app.js` LOC (current) | **587** (post final shell reduction; `app-compat.js` = **1,762** LOC; was 12,456 post Phase 5 cut #1) |
...
| 24 | Parked — settings select compatibility exports | `refactor/app-js-decompose-settings-select-compat-parked` | `/Users/emilionunezgarcia/Job-Bored-worktrees/appjs-settings-select-compat-parked` | **FOLDED INTO FINAL SHELL / PARKED CLEAN** `302f05b` | Preserved unexpected integration dirty edits before final-shell dispatch. Public-global audit proved the settings selectors are required by `registerBridgeHosts()`, so the fix was cherry-picked into final shell as `728642a`. Original parked branch remains clean. Gate passed: `node --check app-compat.js && node --check settings-modal.js`, `git diff --check`, exact conflict-marker scan on touched files, pre-commit staged JS syntax, and full `npm test` = **897 pass / 0 fail** after `npm ci --prefix server` |
| 25 | Final shell reduction | `refactor/app-js-decompose-final-shell-reduction-v2` | `/Users/emilionunezgarcia/Job-Bored-worktrees/appjs-final-shell-reduction-v2` | **MERGED** `41d1a79` | Worker commits `728642a` and `e22b9a8`; moved safe remaining compatibility forwarders into `app-compat.js`, updated source-text tests to follow ownership, and left true shell state/bridge context in `app.js`. Branch and post-merge gates passed: touched JS syntax, `git diff --check`, exact conflict-marker scan, full `npm test` = **897 pass / 0 fail**, and LOC report; `app.js` = **587**, `app-compat.js` = **1,762** |
...
leaving thin delegates in place for the next inspection boundary. C7
`app-compat.js` then landed from the thin-wrapper writer worktree, moving the
already-extracted compatibility delegates out of `app.js` while preserving
browser globals and source-text tests. Session 31 final shell reduction then
landed from its own writer worktree, reducing `app.js` to **587** LOC while
moving safe remaining compatibility forwarders into `app-compat.js` and keeping
the true shell state, bridge registration context, and startup behavior in
`app.js`. Session 18 live Cursor audit also found
...
| `ea20c62` | `sheets-read-load.js` cache key bump in `index.html` | no `app.js` LOC change | green (focused 3 pass) |
| `4507074` | `css/legacy-discovery.css` split | no `app.js` LOC change | green (904 pass) |
...
| 36 | Settings select compatibility exports | **FOLDED INTO FINAL SHELL / PARKED CLEAN** | orchestrator rescue | `appjs-settings-select-compat-parked` | Branch `refactor/app-js-decompose-settings-select-compat-parked`; commit `302f05b`; unexpected integration dirty code preserved and verified, then cherry-picked into final shell as `728642a` after public-global audit confirmed the selectors are required |
| 37 | final shell reduction | **DONE** | writer pane | `appjs-final-shell-reduction-v2` | Branch `refactor/app-js-decompose-final-shell-reduction-v2`; worker commit `e22b9a8`, merged as `41d1a79`; branch and post-merge gates green; `app.js` = **587** LOC |
...
| `/Users/emilionunezgarcia/Job-Bored-worktrees/appjs-thin-wrapper-collapse-v2` | `refactor/app-js-decompose-thin-wrapper-collapse-v2` | Session 28 C7 thin-wrapper collapse writer | merged `402106f`; branch clean at `718c870` |
...
| `/Users/emilionunezgarcia/Job-Bored-worktrees/appjs-settings-select-compat-parked` | `refactor/app-js-decompose-settings-select-compat-parked` | Session 30 settings select compatibility rescue | folded into final shell as `728642a`; original parked branch remains clean at `302f05b` |
| `/Users/emilionunezgarcia/Job-Bored-worktrees/appjs-final-shell-reduction-v2` | `refactor/app-js-decompose-final-shell-reduction-v2` | Session 31 final shell writer | merged `41d1a79`; branch clean at `e22b9a8` |
| `/Users/emilionunezgarcia/Job-Bored-worktrees/appjs-final-shell-boundary-review` | `refactor/app-js-decompose-final-shell-boundary-review` | Session 30 final shell boundary review | read-only support; no implementation edits |
| `/Users/emilionunezgarcia/Job-Bored-worktrees/appjs-final-shell-source-audit` | `refactor/app-js-decompose-final-shell-source-audit` | Session 30 final shell source-text audit | read-only support; no implementation edits |
| `/Users/emilionunezgarcia/Job-Bored-worktrees/appjs-final-shell-qa` | `refactor/app-js-decompose-final-shell-qa` | Session 30 final shell QA gate | read-only support; no implementation edits |
...
| 5 | Discovery remainder | **DONE** (run tracker + relay helpers + scraper ATS config + engine state + status handoff + Apps Script deploy + discovery drawer + ingest URL flow + run orchestration + readiness + setup modals done) |
| 5b | Index decomposition guardrail | **DONE** (`a432bd2`; **894 pass**) |
| 6 | Auth / config / core collapse | **DONE** — final shell reduction merged as `41d1a79`; `app.js` is **587** LOC and gates are green |
...
35. Session 32 missing active Sheet ID guard **landed** — hotfix commit `b3d82d4` keeps `sheets-read-load.js` from building `docs.google.com/spreadsheets/d/null/...` URLs when a persisted OAuth session exists but no active Sheet ID is configured. Commit `ea20c62` bumps the `index.html` `sheets-read-load.js` cache key from `v=1` to `v=2` so browsers fetch the fixed script. Added `tests/sheets-read-load-missing-sheet.test.mjs`. Gate passed: `node --check sheets-read-load.js && node --check tests/sheets-read-load-missing-sheet.test.mjs`, `git diff --check`, exact conflict-marker scan, focused regression test **1 pass / 0 fail**, focused index/cache-key gate **3 pass / 0 fail**, and full `npm test` **898 pass / 0 fail**.
36. Session 33 blank shell diagnostics **landed** — hotfix commit `26733f9` adds secret-safe startup logging for early `window.error` / `unhandledrejection`, missing bootstrap host functions, sheet access gate visibility, and the footer-only blank shell condition where `#dashboard`, `#setupScreen`, and `#sheetAccessGateScreen` are all hidden. Added `tests/startup-diagnostics.test.mjs`. Server log check: `/private/tmp/devserver.log` showed the app shell and assets returning 200 with no visible server exception or 404 in the inspected tail. Gate passed: `node --check app-bootstrap.js`, `node --check sheet-access-setup.js`, `node --check tests/startup-diagnostics.test.mjs`, `git diff --check`, exact conflict-marker scan, focused diagnostics test **3 pass / 0 fail**, static server probes for `/`, `/app-bootstrap.js`, and `/sheet-access-setup.js` returned **200**, and full `npm test` **901 pass / 0 fail**.
37. Session 34 auth prepaint guard fix **landed** — hotfix commit `5029580` fixes the footer-only blank shell caught by the session 33 logs. Root cause: `html.auth-prepaint-dashboard` stayed active after bootstrap, so the CSS forced `#sheetAccessGateScreen` hidden while a premature `loadAllData()` set `#dashboard` inline `display:none`; the v2 region gate then hid the real dashboard sections. The fix releases the prepaint guard when runtime code owns screen visibility, defers initial data load to auth restore when a persisted OAuth session exists but the in-memory token is not populated yet, and improves IMG resource-error logging to include the failed image URL. Gate passed: `node --check app-bootstrap.js`, `node --check sheet-access-setup.js`, `node --check tests/startup-diagnostics.test.mjs`, `git diff --check`, exact conflict-marker scan, focused diagnostics test **3 pass / 0 fail**, and full `npm test` **901 pass / 0 fail**.
38. Session 35 auth token bridge fix **landed** — hotfix commit `f0462b3` fixes the follow-up empty-data shell from the session 34 logs. Root cause: `app.bootstrap.host` omitted `getAccessToken`, so bootstrap could not observe the restored in-memory token and stayed in the deferred data-load path after OAuth restore. The fix exposes `getAccessToken` on the bootstrap bridge, adds Sheets load progress diagnostics (`sheets-read:load:start`, `parsed`, `complete`), and bumps the runtime cache keys for `sheets-read-load.js`, `sheet-access-setup.js`, `app-bootstrap.js`, and `bridge-registry.js`. Asset check: the reported mascot WebP URLs exist on disk and return **200** from the local static server. Gate passed: touched JS syntax, `git diff --check`, exact conflict-marker scan, focused startup/sheets/index test **7 pass / 0 fail**, and full `npm test` **902 pass / 0 fail**.
39. Session 36 active Sheet ID bridge fix **landed** — hotfix commit `44cab51` fixes the zero-data state from the session 35 logs where auth was restored but `sheets-read:load:start` still reported `sheetIdState: 'missing'`. Root cause: `app-config-core.getActiveSheetId()` read through `JobBoredApp.core.host.getSHEET_ID`, but the core host did not expose the live `SHEET_ID` accessors after the bridge split. The fix exposes `getSHEET_ID` / `setSHEET_ID` on `app.core.host`, falls back to the resolved URL/config Sheet ID when the live bridge is not ready, and bumps `app-config-core.js` / `bridge-registry.js` cache keys. Gate passed: `node --check app-config-core.js`, `node --check bridge-registry.js`, `node --check tests/startup-diagnostics.test.mjs`, `git diff --check`, exact conflict-marker scan, focused startup/sheets/index test **8 pass / 0 fail**, and full `npm test` **903 pass / 0 fail**.
40. Session 37 `index.html` static modal partial extraction **landed** — refactor commit `dd22491` moves nine contiguous static modal/wizard surfaces into `partials/`: onboarding wizard, discovery runs modal, expired review modal, settings modal, scraper setup modal, profile/materials modal, LinkedIn capture modal, resume/draft generation modals, and ingest manual modal. This is a move-only extraction: fully expanded HTML from the new includes is byte-identical to `HEAD:index.html` before the cut. On-disk `index.html` dropped from **4,148 LOC** to **1,434 LOC** (**65.4% decrease**) while expanded browser-facing markup remains **6,194 lines**. Current monolith scan after the cut: largest root app/source files are `css/legacy-discovery.css` **3,757**, `discovery-wizard-ui.js` **2,837**, `settings-profile-tab.js` **2,472**, `css/legacy-materials.css` **2,255**, `role.css` **2,068**, `css/legacy-brief.css` **2,040**, `role-materials.js` **1,979**, `dev-server.mjs` **1,937**, `style.css` **1,872**, `pipeline.js` **1,842**, and `app-compat.js` **1,762**. Gate passed: expanded HTML byte-identity check, `node scripts/assemble-index.mjs`, `git diff --check`, exact conflict-marker scan, focused index/onboarding/expired/discovery/ingest test **64 pass / 0 fail**, and full `npm test` **903 pass / 0 fail**.
41. Session 38 legacy discovery CSS split **landed** — worker branch `refactor/legacy-discovery-css-split`, worktree `/Users/emilionunezgarcia/Job-Bored-worktrees/legacy-discovery-css-split`, worker commit `68689ee`, merge `4507074`. The old `css/legacy-discovery.css` monolith (**3,757 LOC**) was split into seven cascade-ordered files: `legacy-discovery-setup-wizard.css` **1,191**, `legacy-profile-modal.css` **1,039**, `legacy-settings-profile.css` **389**, `legacy-discovery-runs.css` **449**, `legacy-discovery-drawer.css` **456**, `legacy-fit-profile-overlay.css` **126**, and `legacy-discovery-coachmark.css` **101**. The largest discovery CSS file is now **68.3% smaller** than the old monolith, and reconstruction of the seven files plus boundary separators is byte-identical to the removed stylesheet. `index.html` now loads the seven files in the same cascade position and remains under the decomposition target at **1,440 LOC**; assembled HTML expands to **6,200 lines**. Branch gate passed: `node --check app.js`, `node --check tests/index-html-size.test.mjs`, CSS reconstruction proof, `node scripts/assemble-index.mjs`, `git diff --check refactor/app-js-decompose...HEAD`, exact conflict-marker scan, focused index test **3 pass / 0 fail**, and full `npm test` **904 pass / 0 fail** after `npm install --prefix server` in the worker worktree. Post-merge integration gate passed the same syntax/reconstruction/assembly checks, `git diff --check HEAD^1...HEAD`, exact conflict-marker scan, focused index test **3 pass / 0 fail**, and full `npm test` **904 pass / 0 fail**. Current monolith scan after the cut: largest root app/source files are `discovery-wizard-ui.js` **2,837**, `settings-profile-tab.js` **2,472**, `css/legacy-materials.css` **2,255**, `role.css` **2,068**, `css/legacy-brief.css` **2,040**, `role-materials.js` **1,979**, `dev-server.mjs` **1,937**, `style.css` **1,872**, `pipeline.js` **1,842**, `app-compat.js` **1,762**, and `discovery-wizard-shell.js` **1,702**. Next visible-app monolith candidates are `discovery-wizard-ui.js` or `css/legacy-materials.css`; next settings/logic candidate is `settings-profile-tab.js`.
...
Active implementation lane: **none**. Final shell reduction is merged, and the
final-shell support lanes remain read-only/clean unless explicitly reassigned.
...
Result: Final shell reduction merged as `41d1a79` from worker commits `728642a`
and `e22b9a8`. Branch and post-merge integration gates passed, `app.js` is
**587** LOC, and `app-compat.js` is **1,762** LOC. The originally documented
app.js refactor plan is complete for inspection.
