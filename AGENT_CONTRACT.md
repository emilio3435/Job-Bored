...
- **`POST /runs/:runId/cancel`** (header `x-discovery-secret`, no body) stops a live async discovery run of this worker process: it aborts the run's in-flight work, blocks any Sheet write the run has not started yet, waits for the run to stop (up to 15 s), and only then writes `failed` with `error` `Cancelled by user.` and the DiscoveryRuns row. Answers: `200 { ok: true, runId, cancelled, run }` (`run` is run-status.v1); `cancelled` is `true` normally and `false` when the run finished before it saw the abort, in which case `run` is its real `completed` status; `401` without the secret; `404` `run_not_found`; `409` `run_already_terminal` (with the finished `run`); `409` `run_not_cancellable` when the run is not live in this process (a synchronous run, or one started before a restart).
