...
function sendStaticBody(req, res, contentType, body) {
  const headers = {
...
async function serveStatic(urlPath, res, req) {
  ...
  try {
    ...
    if (ext === ".html") {
      ...
      sendStaticBody(req, res, ct, data);
      return;
    }
...
function createRequestHandler({ currentPort, logger, discoveryWorkerStarter }) {
  ...
  return (req, res) => {
    ...
    serveStatic(pathname, res, req).then(() => {
