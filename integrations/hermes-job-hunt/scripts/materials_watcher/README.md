...
- Watch root: `~/.hermes/job-hunt/applications` by default.
- Input: `~/.hermes/job-hunt/applications/<slug>/pending.json`.
...
Launchd runs with `PYTHONPATH=~/.hermes/job-hunt/scripts`.

## Install

```bash
~/.hermes/job-hunt/scripts/materials_watcher/install-launchd.sh
```

## Uninstall

```bash
~/.hermes/job-hunt/scripts/materials_watcher/uninstall-launchd.sh
```
