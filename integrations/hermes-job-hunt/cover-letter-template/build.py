...
SRC_COVER_HTML   = HERE / "cover-letter.html"
...
ICLOUD_COVER_PDF  = ICLOUD_DIR / COVER_PDF_NAME
DESKTOP_COVER_PDF = DESKTOP_DIR / COVER_PDF_NAME
...
def render_with_playwright(src_html: Path, out_path: Path) -> None:
    """Launch headless Chromium, load src_html, print to PDF."""
    try:
...
def main() -> int:
    ...
    render_with_playwright(SRC_HTML, ICLOUD_PDF)
    ...
    if SRC_COVER_HTML.exists():
        print()
        print(f"→ Rendering {SRC_COVER_HTML.name} → PDF…")
        render_with_playwright(SRC_COVER_HTML, ICLOUD_COVER_PDF)
        print(f"✓ Wrote {ICLOUD_COVER_PDF}")
        shutil.copyfile(ICLOUD_COVER_PDF, DESKTOP_COVER_PDF)
        print(f"✓ Copied to {DESKTOP_COVER_PDF}")
        size = ICLOUD_COVER_PDF.stat().st_size
        print(f"  size: {size/1024:.1f} KB")
