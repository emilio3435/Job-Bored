...
(function (root) {
  ...
  function renderWorkshop(mount, vm) {
    ...
    if (mount.classList && mount.classList.contains("workshop")) {
      if (!mount.getAttribute("aria-label")) mount.setAttribute("aria-label", "Workshop");
      mount.innerHTML = innerHtml;
    } else {
      mount.innerHTML = '' +
        renderModeDivider() +
        '<aside class="workshop" aria-label="Workshop">' + innerHtml + '</aside>';
    }
