/* ============================================================
   ...
   Public API:
     window.JobBoredAts.score({ jd, draft }) => number
     window.JobBoredAts.analyze({ jd, draft }) => {
...
(function (root) {
  ...
  var W = {
    keywordCoverage: 0.60, // % of JD top-terms present in draft
    toneMatch: 0.25,       // action verbs + first-person ratio
  ...
  function firstPersonRatioScore(text) {
    var words = String(text || "").toLowerCase().match(/\b[\w']+\b/g) || [];
    if (!words.length) return 0;
    var hits = 0;
    for (var i = 0; i < words.length; i++) {
      if (/^(i|me|my|mine|we|our|ours|us)$/.test(words[i])) hits++;
    }
  ...
  function toneScore(draftTokens, draft) {
    ...
    var actionPct = Math.min(100, (hits / ideal) * 100);
    var firstPersonPct = firstPersonRatioScore(draft);
    return Math.round(actionPct * 0.7 + firstPersonPct * 0.3);
  }

  /** Full deterministic scorecard used by letter view-models. */
  function analyze(input) {
    ...
    var tone = toneScore(draftToks, draft);
  ...
  /** Public deterministic scalar score. */
  function score(input) {
    return analyze(input).score;
  }

  /* ---------- expose ---------- */
  root.JobBoredAts = root.JobBoredAts || {};
  root.JobBoredAts.score = score;
  root.JobBoredAts.analyze = analyze;
  root.JobBoredAts.scoreDetails = analyze;
  root.JobBoredAts._internal = {
    ...
    toneScore: toneScore,
    firstPersonRatioScore: firstPersonRatioScore,
  ...
  (function selfTest() {
    try {
      ...
      var a = analyze({ jd: jd, draft: draft });
      var b = analyze({ jd: jd, draft: draft });
      var scalar = score({ jd: jd, draft: draft });
      var scalarAgain = score({ jd: jd, draft: draft });
      var ok =
        a && typeof a.score === "number" &&
        a.score >= 0 && a.score <= 100 &&
        typeof scalar === "number" && scalar === a.score && scalarAgain === scalar &&
      ...
      // empty input shape
      var z = analyze({ jd: "", draft: "" });
      var emptyOk =
        z && z.score === 0 && z.keywordCoverage === 0 &&
        z.length.words === 0 && Array.isArray(z.hits) && Array.isArray(z.misses);
      if (typeof console !== "undefined") {
        if (ok && emptyOk && console.log) {
          console.log("[ats-score] self-test pass");
        } else if (console.warn) {
          console.warn("[ats-score] self-test fail", { a: a, b: b, z: z });
        }
      }
    } catch (e) {
      // never throw on load
